...
class Sphere(Shape):
    radius = 1
    kEpsilon = .1
    def __init__(self, center = Point3D(), radius = 1, material = Material()):
        if type(center) == type(()):
            self.c.x = center[0]
            self.c.y = center[1]
            self.c.z = center[2]
    ...
    def hit(self, Ray):
        ...
        if disc < 0.0:
            ...
            if t > self.kEpsilon:
                intersection_point = Ray.o + t * Ray.d   
                normal = Vector3D((intersection_point.x - self.center.x) / self.radius, (intersection_point.y - self.center.y) / self.radius, (intersection_point.z - self.center.z) / self.radius)             
                return RayIntersection(self, normal, intersection_point)
                #return (self, normal, intersection_point) 
    ...
    def __str__(self):
        return "Sphere - X:" + str(self.center.x) + " Y:" + str(self.center.y) + " Z:" + str(self.center.z) + "  With Radius " + str(self.radius)
    ...
    def hit(self, Ray):
        ...
        for face in self.faces:
            ...
            if t > 0:
                ...
                if (hit_point.x > maxx or hit_point.x < minx) and (hit_point.y > maxy or hit_point.y < miny) and (hit_point.z > maxz or hit_point.z < minz):
